const Navbar = () => {
  return (
    <nav className="h-full w-[224px] py-4 px-2.5 flex flex-col gap-4 bg-[#1E2640]">
...
const NavHeader = () => {
  return (
    <div className="h-[42px] px-2 flex gap-3 items-center">
      <img className="h-[39px] w-[39px] rounded" src="./assets/company-logo.png" alt="company logo" />
      ...
      <img className="h-5 w-5 cursor-pointer" src="./assets/chevron-down.svg" alt="down arrow" />
    </div>
  );
};
...
const NavButton = ({ icon, name, isActive = false }: { icon: string; name: string; isActive?: boolean }) => {
  return (
    <div
      className={`flex items-center h-9 w-full px-4 item-center self-start gap-3 hover:bg-[#353C53] rounded ${
        isActive && "bg-[#353C53]"
      }`}
    >
      <img className={`w-5 h-5 ${isActive ? "opacity-100" : "opacity-80"}`} src={icon} alt={name} />
      <p className={`text-white text-sm font-medium leading-5 select-none ${isActive ? "opacity-100" : "opacity-80"}`}>
        {name}
      </p>
    </div>
  );
...
const NavBottom = () => {
  return (
    <div className="flex flex-col w-[192px] items-start self-center bg-[#353C53] rounded px-3 py-1.5 gap-2.5">
